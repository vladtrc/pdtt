...
func (rt *Runtime) addSetEvents(v RecordStmt, t float64) error {
	...
	for _, fd := range v.Fields {
		if !fd.Set {
			return fmt.Errorf("line %d: record %s already exists; only `set` fields may follow", fd.Line, v.Name)
		}
		fd := fd
...
func (rt *Runtime) expandRow(row Row, w winState, start, dur float64, binds map[string]Value) error {
	...
	case "write":
		return rt.expandVerbSimple(row, w, mkAnim, func(e *Entity, a *Anim) {
			a.Targets = []Ref{RevealRef{E: e}}
			opTarget := FieldRef{E: e, F: e.field("opacity")}
	...
	case "fade_in":
		dir := Vec{}
		if row.Op.Arg != "" {
			if d, ok := namedVecs[row.Op.Arg]; ok {
				dir = d
			}
		}
		return rt.expandVerbSimple(row, w, mkAnim, func(e *Entity, a *Anim) {
			op := FieldRef{E: e, F: e.field("opacity")}
			a.Targets = []Ref{op, OffsetRef{E: e}}
			a.Update = func(a *Anim, rt *Runtime, u float64) {
				op.Set(u)
				e.Offset = dir.Mul(0.45 * (1 - u))
			}
		})
	case "morph":
		return rt.expandMorph(row, w, mkAnim)
	}
	return fmt.Errorf("line %d: unknown op kind %q", row.Line, row.Op.Kind)
}
...
func fillTween(a *Anim, ref Ref, rhs Expr, elemIdx, elemN int) {
	...
	a.Start = func(a *Anim, rt *Runtime) error {
		...
		if list, ok := goal.([]Value); ok && elemIdx >= 0 && len(list) == elemN {
			goal = list[elemIdx] // collections pair by position
		}
		cur := ref.Get()
		if cur != nil {
